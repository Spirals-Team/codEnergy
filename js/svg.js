class Sunburst {
    ...
    power() { return this.cpu + this.disk; }
    json() {
        let object = {};
        let children;
...
class SVG {
    static createSunburst(d3, jquery, textures, colors, sunburstJson) {
        ...
        function arcTween(d) {
            ...
            let xd = d3.interpolate(xSunburst.domain(), [d.x, d.x + d.dx]), yd = d3.interpolate(ySunburst.domain(), [d.y, 1]), yr = d3.interpolate(ySunburst.range(), [d.y ? 20 : 0, radius]);
            return function (d, i) {
                return i
        ...
        let chart = jquery('#sunburst'), aspect = chart.width() / chart.height(), container = chart.parent();
...
function changeRun(config, d3, jquery, textures, chroma, chromaPalette) {
    ...
    let colorsIWH = palette.generate(chroma, methods.size, function (color) {
        let hcl = color.hcl();
