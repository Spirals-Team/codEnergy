  ...
  umd(function() {
    return {
      palette: function() {
        ...
        palette.generate = function(chroma, colorsCount, checkColor, forceMode, quality, ultra_precision, distanceType){
      		// Default
      		...
      		if(forceMode){
      			...
      			function checkLab(lab){
      				...
      				return palette.validateLab(lab) && checkColor(color);
      			}
      			...
      			while(steps-- > 0){
      				...
      				for(i=0; i<colors.length; i++){
      					...
      					for(j=0; j<i; j++){
      						...
      						var d = palette.getColorDistance(colorA, colorB, distanceType)
      						if(d>0){
      			...
      			while(steps-- > 0){
      				...
      				for(i=0; i<colorSamples.length; i++){
      					...
      					for(j=0; j<kMeans.length; j++){
      						var kMean = kMeans[j];
      						var distance = palette.getColorDistance(lab, kMean, distanceType);
      					...
      					if(count!=0 && checkColor2([candidateKMean[0], candidateKMean[1], candidateKMean[2]]) && candidateKMean){
      						...
      						if(freeColorSamples.length>0){
      							...
      							for(i=0; i<freeColorSamples.length; i++){
      								var distance = palette.getColorDistance(freeColorSamples[i], candidateKMean, distanceType);
      								if(distance < minDistance){
      									minDistance = distance;
      									closest = i;
      								}
      							...
      							var closest = -1;
      							for(i=0; i<colorSamples.length; i++){
      								var distance = palette.getColorDistance(colorSamples[i], candidateKMean, distanceType)
      								if(distance < minDistance){
      									minDistance = distance;
      									closest = i;
      								}
        ...
        palette.diffSort = function(colorsToSort, distanceType) {
      		// Sort
        ...
        palette.getColorDistance = function(lab1, lab2, _type) {
          var type = _type || 'Default'
      		...
      		function distanceColorblind(lab1, lab2, type) {
      			var lab1_cb = palette.simulate(lab1, type);
      			var lab2_cb = palette.simulate(lab2, type);
        ...
        palette.simulate_cache = {};
        palette.simulate = function(lab, type, _amount) {
          // WARNING: may return [NaN, NaN, NaN]
      		...
      		var cache = palette.simulate_cache[key]
      		if (cache) return cache

      		// Get data from type
      		var confuse_x = palette.confusionLines[type].x;
      		var confuse_y = palette.confusionLines[type].y;
      		var confuse_m = palette.confusionLines[type].m;
      		var confuse_yint = palette.confusionLines[type].yint;
      		...
      		palette.simulate_cache[key] = result;
      		return result;
        };
        palette.validateLab = function(lab) {
      		var LAB_CONSTANTS = {
